...
const billingSourceOptions = [
  { value: 'E_INV_DAY', label: 'eInvDay / 10 - Inverter output' },
  { value: 'E_TO_USER_DAY', label: 'eToUserDay / 10 - To user' },
  { value: 'E_CONSUMPTION_DAY', label: 'eConsumptionDay / 10 - Consumption' },
];
...
export default function AdminLuxPowerPage() {
  ...
  async function handleTest() {
    ...
    try {
      ...
      setMessage('Đã test LuxPower và lưu raw/debug snapshot an toàn.');
    } catch (requestError) {
  ...
  async function handleDelete() {
    if (!selectedConnection || !window.confirm(`Lưu trữ connection "${selectedConnection.accountName}"?`)) {
      return;
    }
    setSaving(true);
    setMessage('');
  ...
  return (
    <div className="space-y-5">
      ...
      <div className="grid gap-5 xl:grid-cols-[320px_minmax(0,1fr)]">
        <SectionCard
          title="Danh sách LuxPower"
          eyebrow="Liên kết plant LuxPower với system và contract trong Moka"
          dark
        >
          ...
          <div className="grid gap-3">
            ...
            {!connections.length ? (
              <div className="rounded-[22px] border border-dashed border-white/10 px-4 py-8 text-sm text-slate-400">
                Chưa có LuxPower connection nào.
              </div>
            ) : null}
          </div>
        </SectionCard>
        ...
        <div className="space-y-5">
          <SectionCard
            title={mode === 'create' ? 'Tạo LuxPower connection' : 'Cấu hình LuxPower'}
            eyebrow="Giữ secret ở backend, đồng bộ raw -> normalized -> billing"
            dark
          >
          ...
          {selectedConnection ? (
            <>
              <SectionCard title="Trạng thái pipeline" eyebrow="Auth -> Metrics -> Link -> Billing" dark>
                <div className="grid gap-3 md:grid-cols-4">
                  {[
                    ...
                    },
                    {
                      ...
                      note: `${selectedConnection.normalizedMetrics?.length || 0} normalized rows`,
                    },
                    {
                      label: 'Plant linked',
              ...
              <div className="grid gap-5 xl:grid-cols-2">
                <SectionCard
                  ...
                  eyebrow="Các field công suất giữ nguyên đơn vị W"
                  dark
                >
                  <div className="grid gap-3 sm:grid-cols-2">
                    <div className="portal-card-soft p-4">
                      <p className="text-xs uppercase tracking-[0.2em] text-slate-500">PV power</p>
                    ...
                    <div className="portal-card-soft p-4">
                      <p className="text-xs uppercase tracking-[0.2em] text-slate-500">Load power</p>
                      <p className="mt-2 text-2xl font-semibold text-white">
                        {renderMetricValue(
                    ...
                    <div className="portal-card-soft p-4">
                      <p className="text-xs uppercase tracking-[0.2em] text-slate-500">Grid power</p>
                      <p className="mt-2 text-2xl font-semibold text-white">
                        {renderMetricValue(
                ...
                <SectionCard
                  title="Link system + billing"
                  eyebrow="Nguồn normalized dùng để tính hóa đơn tháng"
                  dark
                >
                  ...
                  <div className="mt-4 rounded-[18px] border border-white/8 bg-white/[0.03] px-4 py-4 text-sm text-slate-300">
                    <div className="flex items-center gap-3">
                      ...
                      <p className="font-medium text-white">
                        Flow đang dùng: monitor LuxPower {'->'} normalized metric {'->'} linked
                        system {'->'} monthly billing
                      </p>
                    </div>
                  </div>
                </SectionCard>
              ...
              <SectionCard
                title="Debug raw + normalized"
                eyebrow="Kiểm tra scale 0.1, raw value và normalized value trước khi tính bill"
                dark
              >
                <div className="grid gap-5 xl:grid-cols-2">
              ...
              <SectionCard title="Lịch sử sync" eyebrow="Login, fetch, normalize, billing" dark>
                <div className="space-y-3">
                  {logs.map((log) => (
                    <div
                      key={log.id}
