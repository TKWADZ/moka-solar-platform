import { ADDITIONAL_MARKETING_PAGES } from './marketing-page-extensions';

export type DefaultMarketingPage = {
  key: 'home' | 'about' | 'pricing' | 'solutions' | 'contact';
  name: string;
  description: string;
  published: boolean;
  sortOrder: number;
...
export const DEFAULT_MARKETING_PAGES: DefaultMarketingPage[] = [
  {
    key: 'home',
    name: 'Trang chủ',
    ...
    content: {
      vi: {
        ...
        newsroomSection: {
          ...
          title: 'Xuất bản case study, nội dung kinh doanh và ghi chú vận hành ngay từ cùng một hệ thống.',
      ...
      vi: {
        ...
        cards: [
          ...
          },
          {
            ...
            body: 'Cùng backend này có thể mở rộng sang mobile app, inverter API thật, nhắc nợ qua Zalo/SMS và mô hình multi-tenant trong tương lai.',
          },
        ],
      },
      en: {
  ...
  ...ADDITIONAL_MARKETING_PAGES,
];

export function buildDefaultMarketingPages() {
  return DEFAULT_MARKETING_PAGES.map((page) =>
    JSON.parse(JSON.stringify(page)) as DefaultMarketingPage,
  );
}
